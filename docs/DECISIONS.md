...
## 2026-06-25 — Creator Room: aesthetics, onboarding, service location

From a 13-agent research workflow (see docs/CREATOR_ROOM_PLAN.md).

| # | Decision | Choice | Rationale |
|---|----------|--------|-----------|
| D35 | Service location | The Creator Room HTTP API lives in **`src/comms-service/`** (FastAPI, :8787), separate from the team's MCP service in `src/python-service/` (:9000). Frontend reaches it via `NEXT_PUBLIC_COMMS_API_URL` (defaults to `http://localhost:8787` on localhost). | Two concerns, two services; avoids colliding with the MCP/analysis work. |
| D36 | 3D clay aesthetic | Vanilla three.js (WebGLRenderer) in the sandboxed iframe + a **clay post-stack**: AgX tone mapping, GTAO ambient occlusion → soft bloom → SMAA, clay materials (roughness 0.92, metalness 0). NOT R3F at runtime (can't transpile into a static-export srcdoc). | Convergent research winner; AO + AgX are ~80% of the clay look. Kit-bashed GLBs (D38) are the next aesthetic step. |
| D37 | Onboarding | Replace the big form with a **3-question wizard** (niche → vibe → name+pet) + **LLM autofill** (`/api/creator-room/autofill`) that expands the seed into a full profile; deterministic coercion + fallback so it never dead-ends. Auto-opens on first visit; big form kept behind "Edit details". | Decouples "questions asked" (3) from "fields filled" (~27). Conversational one-at-a-time ≈ 3× single-page completion. |
| D38 | Interactive 3D (next) | The 3D mode becomes a **kit-bashed CC0-GLB diorama** placed by a schema-constrained Claude **layout** call (`/api/creator-room/layout`), cached by profile hash. Reject single-image-to-3D (un-pickable blob, kills referral hotspots) and Spline (GUI-authored binary). | Only candidate with genuine per-object hotspots + deterministic data-driven layout + ~$0/room after cache. Backlog (not yet built). |

## 2026-06-26 — UGC review loop (agency feedback)

Rescope to widen the app: an agency hands briefs to creators, collects final videos,
and reviews each delivery against its brief. See `docs/ugc-review-loop.md`.

| # | Decision | Choice | Rationale |
|---|----------|--------|-----------|
| D39 | New workflow | Add an agency **UGC review loop** (Brief → Deliver → Review → Coach) on the existing analysis spine, alongside (not replacing) the solo-creator preproduction flow. | Agencies already keep footage in the cloud (lowest-friction segment per the Avlana insights); short UGC clips analyze near-instantly; the value is a pre-render "magical moment" — the brief-vs-final diff + a copy-paste coaching note. |
| D40 | The review "brain" is the agent, not a backend | The review is **the agent reasoning** over the brief text + the reference analysis + the final analysis, then building ordinary frames + text/video blocks. **No scoring service, no new tables, no new shapes.** The contract (rubric + frame layout) is a prompt: `review_workflow.py:UGC_REVIEW_GUIDE`, injected into the MCP `instructions` (covers Claude Code *and* the embedded agent) + a `.claude/skills/ugc-review` runbook. | Per the user's steer and the "treat video as code / models keep getting better at editing our own data structures" insight: let the model reason. Everything stays in the fundamental blocks (text/video/image in a frame); add a new element type only if genuinely needed (a chart is the one candidate, deferred). |
| D41 | Local-only DB for the concept test | All work targets **`localhost:5432/sunstead`** (`.env`, schema already loaded). The external **Aiven** DB is never touched. The simplification means **zero DDL**, so no migration can reach Aiven. | Test the concept locally and iterate freely; keep `src/database/schema.sql` canonical; harden/port later. |
| D42 | Reference ingestion is the known blocker | CSV references are Instagram/TikTok/Drive URLs; the row-2 IG reference currently fails to download (yt-dlp, exit 1). The review **degrades honestly** when a reference is missing/failed rather than fabricating a comparison. Real fix (cookies or direct upload) tracked separately. | Don't block the review contract on ingestion; don't fake a pass on data we don't have. |

## 2026-06-26 — Agency surface: an operational dashboard (branch `feat/ugc-agency-dashboard`)

Research (`docs/ugc-coaching-exploration.md`) concluded the validated agency job is an
**operational pipeline** (a queue of deliveries → a Slack-ready note → scores trending),
not a thinking surface. The infinite canvas is the right home for **solo-creator ideation**
and the wrong default for **agency coaching**. So: decouple the analysis core from the
surface and build the dashboard as a separate, thin client at `/agency`.

| # | Decision | Choice | Rationale |
|---|----------|--------|-----------|
| D43 | Primary agency surface | A **dashboard** (`src/canvas-ui/app/agency`): **roster** table (creators × hook/tone/pacing/reference trend) → **creator detail** (improvement-over-time chart + deliveries queue) → **delivery detail** (verdict + per-dimension scorecard + strengths/missing + a copy-paste **coaching note**). The canvas stays as an optional rich deep-dive, not the home. | A canvas has no queue/triage/sort/trend column; 50 deliveries/mo across a roster is a stream, not a whiteboard. Exports as a thin 105 kB route vs the canvas's ~670 kB — the "thin, swappable client" thesis, in bytes. |
| D44 | Review brain, server-side for non-agent surfaces | `review_generate.py` runs the **same reasoning** as the canvas agent as one **headless Claude call** (forced tool-use → structured verdict/scores/dimensions/strengths/missing/note), over the **same compact signals** `compare_components` gathers (reused `tools.review._video_brief`). A background watcher generates the review once analysis finishes; the dashboard polls. | Refines **D40**: the canvas keeps the agent-reasons-the-note path; a button-driven dashboard needs the note generated without a chat agent. The diff/signals layer is shared so both surfaces judge identically. |
| D45 | Persist scores → a `reviews` table | New `reviews` table (operational record: creator, delivery, reference, brief, status, verdict, scores, dimensions, strengths, missing, note) + a `talent` creator kind for the roster. Migration `003_agency_reviews.sql`; `schema.sql` canonical. | Refines **D40/D41** ("no new tables / no DDL"): improvement-over-time is *the retention hook* and needs persisted, comparable scores. Still local-first — apply 003 to `localhost`; Aiven untouched until productionised. |
| D46 | Ingestion is upload-first | The delivery POST takes a base64 **file upload** (saved under `python-service/uploads/`, analysed in place — the worker skips yt-dlp when `source_url` is `upload://…` and a local file exists), with a **URL** path as best-effort fallback. Reference videos stay URL-based (best-effort). | Flips **D42** from a blocker to a reference-only concern: the agency already has the delivery file, so we never scrape a walled post for the thing being graded. Dependency-free (base64 in JSON; no `python-multipart`). |
